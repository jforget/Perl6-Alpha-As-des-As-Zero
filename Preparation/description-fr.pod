...
de transition (page de départ, manoeuvre) -> page d'arrivée,
stockée dans un fichier JSON. I<Handy Rotary> comporte 25 manoeuvres par page.
Recopier un livret avec 222 pages et 25 manoeuvres dans un fichier JSON, soit 5550 entrées,
est fastidieux, donc sujet à erreur. Et il faut le faire pour au moins
deux livrets. Même dans le cas de I<Wingleader>, qui a seulement 13 manoeuvres
...
de transitions (page de départ, manoeuvre) -> page d'arrivée
...
peuvent être alors générés en décrivant les 13 à 26 manoeuvres disponibles
...
Ce fichier contient la description des manoeuvres du drone, ainsi que les
pages où le drone peut S<« tirer »> sur l'épervier (c'est-à-dire le filmer). Le fichier ne contient aucune
transition (page de départ, manoeuvre) -> page d'arrivée.
...
et la liste des transitions manoeuvre -> page d'arrivée
...
transitions (page de départ, manoeuvre) -> page d'arrivée.
...
et qui déterminait pour chaque manoeuvre la page d'arrivée.
Très chronophage.

=head3 Exemple

Prenons un engin avec très peu de manoeuvres disponibles.
La manoeuvre A, qui permet un déplacement en crabe vers l'hexagone
voisin à 10 heures, la manoeuvre B qui fait avancer dans l'hexagone
juste devant la position initiale et la manoeuvre C qui permet un
déplacement en crabe vers l'hexagone voisin à 2 heures.

=for html
<img src='hexagons-a-fr.png' alt='Représentation des mouvements possible sur la grille hexagonale' width='30%' />
...
=for html
<img src='hexagons-b.png' alt='Représentation sur la grille hexagonale des pages accessibles depuis la page 187' width='30%' />

I<Copie d'écran personnelle. Les conditions de licence sont les mêmes que pour le texte.>

En examinant les pages nouvellement créées 15, 16 et 17 et en leur appliquant
les manoeuvres A, B et C, le programme constate que sur les 9 tentatives, deux
...
=for html
<img src='hexagons-c.png' alt='Remplissage de la grille hexagonale après la deuxième mise à jour' width='30%' />

I<Copie d'écran personnelle. Les conditions de licence sont les mêmes que pour le texte.>

En examinant les pages nouvellement créées 46, 91 et 107 et en leur appliquant
les manoeuvres A, B et C, le programme
ajoute les enchaînements 

  ( 46, C) -> 91
  (107, A) -> 91

Le programme applique également les manoeuvres A, B et C à rebrousse-poil
...
et toutes les manoeuvres. Tous ces appels sont accumulés dans le
script shell F<maj1>, de sorte que l'on peut refaire la totalité
des itérations en un seul appel de F<maj1>.

Avec 9 manoeuvres, dont une (C<B>) qui consiste à faire du sur-place,
...
en I<ASCII art>, présentant les positions du drone pour toutes les pages connues.
...
L<après la première mise à jour|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Preparation/etape1.pod>
et le second une fois
L<la grille terminée|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Preparation/etape-finale.pod>.
...
manoeuvre. Pour cela, j'ai créé une classe C<Depl.pm6>, pour S<« déplacement »>.
Il s'agit soit du déplacement entraîné par l'exécution d'une manoeuvre, soit
...
=for html
<img src='hexagons-on.png' alt='Représentation de la grille hexagonale dans un repère orthonormé' width='30%' />
...
=for html
<img src='hexagons-o.png' alt='Représentation de la grille hexagonale dans un repère orthogonal' width='30%' />
...
L<Suivez mon regard|https://boardgamegeek.com/boardgame/1711/richthofens-war>.

=for html
<img src='hexagons-n.png' alt='Représentation de la grille hexagonale dans un repère normé' width='30%' />
...
directions C<0> à C<5>, la longueur de chaque pas étant implicitement de 1.
...
les deux avions. On utilise la même notation de C<0> à C<5>, mais sans
...
Car l'hexagone du drone est situé dans la direction C<2> par rapport
