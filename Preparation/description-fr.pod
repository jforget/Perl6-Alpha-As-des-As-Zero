...
Recopier un livret avec 222 pages et 25 manœuvres, soit 5550 entrées,
...
pages où le drone peut tirer sur l'épervier. Le fichier ne contient aucune
...
une collection de la base MongoDB. Il initialise une autre
collection avec deux S<pages :> la page 223, puisqu'elle a un statut
...
encore rencontrées jusqu'à présent, le programme détermine où se situe
cette page dans la grille hexagonale et quelle est l'orientation de
l'avion (ou du drone, ou de l'oiseau, etc).

En outre, le programme applique toutes les manœuvres à chacune des
nouvelles pages. Si le résultat est une page déjà identifiée, le programme
renseigne cette associations (page de départ, manœuvre) → page d'arrivée
...
voisin à 10 heure, la manœuvre B qui fait avancer dans l'hexagone
...
mais on trouve également les noueaux enchaînements :
