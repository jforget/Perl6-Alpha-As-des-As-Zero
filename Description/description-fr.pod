...
Vous devriez plutôt lire la
L<version  Markdown|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Description/description-fr.md>
de
ce document. En  effet, au printemps 2022, j'ai remarqué  que le rendu
de la version POD par Github posait des problèmes dans les paragraphes
comportant  des caractères  au-delà de  U+00FF. Voir  par exemple  les
exemples de code idiomatique dans  le chapitre sur L<la fonctionnalité
de Raku la plus utile|"La fonctionnalité de Raku la plus utile"> ainsi
que le chapitre sur les L<opérateurs Unicode|"Opérateurs Unicode">.
Comparez donc ces paragraphes avec leur
L<équivalent|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Description/description-fr.md#user-content-la-fonctionnalité-de-raku-la-plus-utile>
L<Markdown|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Description/description-fr.md#user-content-opérateurs-unicode>.
...
Simultanément, le Britannique  choisit la manoeuvre en  dessous de S<«
EF »> S<« Cruising> S<Left »> et répond :
...
I<Photo personnelle reprenant une page de Handy Rotary, copyright © 1986 Nova Game Designs, Inc., image copyright © 1986 Emithill limited>
...
le S<« T »> sous le numéro de page dans le livret allemand). Pour des raisons pédagogiques,
...
=item 1 L'avion poursuivi choisit une manoeuvre.

=item 2 Il indique au poursuivant s'il s'agit d'une manoeuvre vers
...
=item 3 L'avion poursuivant choisit une manoeuvre.
...
=head3 Enchaînement des manoeuvres
...
L<ce genre de gazelle|https://fr.wikipedia.org/wiki/Gazelle>,
mais plutôt
L<ce genre de gazelle|https://fr.wikipedia.org/wiki/Sud-Aviation_SA340/341/342_Gazelle>)
...
I<n> si l'avion en a déjà fait une au tour I<n>-1.

Également,  il y  a les  manoeuvres acrobatiques,  qui ne  peuvent être
effectuées au  tour I<n> que  si l'avion  a effectué une  manoeuvre
S<« tout> S<droit »> au tour I<n>-1.
...
=item 7 le type de manoeuvre (normale, tout droit, acrobatique, basse vitesse) si vous utilisez la règle sur les enchaînements de manoeuvres,

=item 8 optionnellement le nombre de points de dégâts infligés à votre adversaire.
...
L<préparation|"Préparation">,  consistant  à  produire  les  livrets  sous  forme
électronique, en générant les fichiers plutôt qu'en tapant la totalité
des enchaînements entre  pages et manoeuvres. Puis il y  a le L<jeu|"Jeu">
...
L<xubuntu|https://xubuntu.fr/> 21.04 "Hirsute Hippo". Je ne vous donne
...
=head3 Considérations sur la sécurité
...
est accessible  uniquement sur la machine  qui l'héberge (C<localhost>
ou C<127.0.0.1>). Si quelqu'un est connecté sur cette machine, il peut
...
et vous pourriez même avoir  l'équivalent d'un déni de service (I<DOS>
ou I<DDOS>).

Finalement, je  n'ai pas cherché  à faire quelque chose  de "packagé",
avec un  fichier configuration global  dans le répertoire  F</etc>, un
fichier de  configuration personnalisé  dans F<$HOME>  et dont  le nom
...
F<launch> lance  une partie  entre un épervier  générique et  un drone
générique. Si vous voulez une  partie entre Plume-Noire et Kévin, vous
devrez éditer le fichier F<launch> pour inhiber certaines lignes et en
réactiver d'autres.

=head3 Définition de la machine

Si  vous utilisez  une machine  virtuelle, pensez  à lui  attribuer au
moins   2048    Mo   de   mémoire   vive.    La   L<documentation   de
...
=head3 Installation de logiciels extérieurs

Avec  le gestionnaire  de paquets  de la  distribution (C<apt-get>  ou
I<Synaptic> dans le cas de xubuntu), installez
L<Emacs|https://www.gnu.org/software/emacs/>,
L<Git|https://git-scm.com/>,
L<curl|https://curl.se/>
et la bibliothèque C<libssl-dev>.

Et vérifiez que vous avez bien L<GnuPG|https://gnupg.org/>.

Pour  Emacs,  c'est  une  préférence  personnelle.  Si  vous  préférez
L<vim|https://www.vim.org/>  ou  L<nano|https://www.nano-editor.org/>,
libre à vous de les installer.  En revanche, les autres logiciels sont
absolument nécessaires.

=head3 Installation de Rakudo

À la  différence de  mon installation de  2018, décrite  au paragraphe
L</"Compilation et installation  de Raku">, la version  de xubuntu est
...
ce paragraphe L</"Compilation et installation de Raku">.
...
C<zef>.   Sur  d'autres   systèmes,  vous   pouvez  avoir   un  script
F<install-zef>.  Il faut  bien entendu  le lancer.  Sinon, suivez  les
instructions dans le
L<dépôt Github de zef|https://github.com/ugexe/zef>.

À noter  une mésaventure  qui m'est  arrivée ultérieurement  sur cette
machine  S<virtuelle  :>  j'ai  voulu  tester  l'accès  à  SQLite  par
l'intermédiaire du
L<module DBIish|https://github.com/raku-community-modules/DBIish>.
L'installation de C<DBIish> par C<zef> a planté, parce qu'il n'y avait
pas de compilateur C. Donc, en plus de Git et C<curl> déjà mentionnés,
pensez à installer C<gcc>, C<binutils> et C<make>.

=head3 Installation de modules Rakudo
...
MongoDB. L'installation  se fait par  C<zef>. Les modules  à installer
sont :
...
C<HTTP::Supply>  produit  un  avertissement.  Mais  c'est  normal  (cf
L<https://github.com/zostay/HTTP-Supply/issues/5>).  Pour  C<Template::Mojo> et
C<Bailador>, il  y a eu d'autres  erreurs que je  n'ai pas eu le  temps de
...
L<paragraphe suivant|"Installation du module MongoDB">.

Le  L<site   de  MongoDB|https://www.mongodb.com/fr>  ne   décrit  pas
...
c'est  la  distribution  de  base  I<Ubuntu>,  pas  les  distributions
dérivées I<Kubuntu>  ou I<xubuntu>.  Néanmoins, j'ai  fait l'hypothèse
...
L<page d'installation|https://docs.mongodb.com/manual/tutorial/install-mongodb-on-ubuntu/>
...
accessible de  l'extérieur. Pour ceci,  consulter F</etc/mongodb.conf>
et vérifier que le fichier contient bien :

  bind_ip = 127.0.0.1
  #port = 27017

=head3 Installation du module MongoDB

Première idée, taper :

  zef install MongoDB

Mauvaise  idée,  le  test  F<t/099-start-servers> plante  et  le  test
F<t/200-Database.t>  boucle. Il  faut donc  interrompre l'installation
avec C<Ctrl-C> et recommencer.
...
=head3 Installation du projet pour l'As des As
...
En  désactivant  certaines  lignes  de  F<launch>  avec  un  dièse  et
...
Pour arrêter  le serveur web, faire  C<Ctrl-C> sur la fenêtre  où vous
l'avez lancé.

=head3 Extension aux véritables jeux de l'As des As

Pour jouer  avec les véritables  protagonistes des jeux de  l'I<As des
As>,  il faut  utiliser les  programmes du  répertoire F<Preparation>.
Initialisez la base de données C<aoa_prep> avec :

  raku init.p6
  sh maj1

Ensuite, pour chaque avion, tapez  un fichier d'initialisation dans le
répertoire F<Preparation/fr> ou F<Preparation/en>. La différence entre
...
c'est-à-dire le fichier F<Preparation/fr/Epervier-init.json>.
...
Prenons  comme  exemple  le  fichier  F<Preparation/fr/DR1-init.json>,
décrivant le Fokker DR.1 de I<Handy Rotary>.

Le nom sera quelque chose comme S<« Fokker> S<DR.1 »>. L'identité sera
une version simplifiée, de nature à s'intégrer sans peine dans un URI.
En  d'autres  termes,  il  faut  éviter  les  C<%20>,  les  C<%27>  et
...
Tout d'abord, attribuez un code à chaque manoeuvre. Pour I<Wingleader>,
pour I<Jet Eagles> et pour  certaines éditions de I<Handy Rotary>, les
codes  sont déjà  attribués,  mais  pour les  autres  jeux, il  faudra
attribuer des codes C<A> jusqu'à ... (jusqu'à C<M> pour I<Wingleader>,
jusqu'à C<Y> pour I<Handy Rotary> et I<Jet Eagles>).

Ainsi donc, les manoeuvres du brevet sont les manoeuvres B<A> et B<J>.
Ce qui nous donne :
...
=for HTML
<img src='Page-48-All.png' alt="Livret allemand page 48" />

En regardant la ligne du haut dans le tableau des manoeuvres, on trouve
la  vitesse :  I<slow> (lent)  pour  la manoeuvre  B<A> et  I<cruising>
(vitesse de croisière) pour la manoeuvre B<J>. Cela se traduit par :
...
à droite (respectivement I<left> I<straight> et I<right>). À l'inverse
de la vitesse,  c'est une donnée importante, utilisée  pour les règles
de poursuite  (I<tailing>). Dans  notre cas, les deux  manoeuvres vont
...
page, la manoeuvre B<A> nous renvoie page 205 et la manoeuvre B<J> nous
renvoie page 23. Consultons les lignes correspondantes dans le fichier
F<Preparation/etape-finale.html>
...
Notons que pour la page 187 du Camel, la manoeuvre B<A> aurait conduit
à la page 198 et la manoeuvre B<J>  à la page 31. Pour ces deux pages,
le fichier F<etape-finale.html> file nous donne :
...
pouvez le voir, nous obtenons les mêmes formules C<5;5> et C<;5>.
...
anglophones et le stocker dans F<Preparation/en/DR1-init.json> au lieu
de F<Preparation/fr/DR1-init.json>. Cela donne :
...
Notons  que F<init-aircraft.raku>  attend  des mots-clés  anglophones,
c'est pour cela que l'on prend le fichier du sous-répertoire F<en>. Le
sous-répertoire F<fr> contient  un fichier JSON avec  des mots-clés en
français, utilisable  pour les  programmes du répertoire  F<Jeu>. Seul
problème, les programmes de ce répertoire ne sont plus maintenus.

Les  deux sous-répertoires  F<en>  et F<fr>  contiennent également  un
fichier F<DR1.html> qui présente les  mêmes données, mais plus faciles
à lire que dans le format JSON.

Ensuite, faites  de même avec  le Sopwith Camel.  Pour les jeux  de la
série I<As  des As>, à  l'exception du I<Starfighter Battle  Book>, il
...
le fichier F<launch> (ou le  dupliquer puis l'adapter) pour lancer des
...
Comme pour les avions, l'attribut C<name> est un nom destiné à être lu
par un  humain, avec si  nécessaire des espaces, des  apostrophes etc.
tandis que l'attribut C<identity> est  un nom compatible avec les URI,
donc  sans caractères  spéciaux tels  que  les C<%20>  et les  C<%27>.
L'attribut  C<aircraft>  est  l'attribut C<identity>  de  l'avion  sur
lequel  le  pilote  vole.  L'attribut C<perspicacity>  est  un  nombre
flottant  entre  0 et  1  et  l'attribut  C<stiffness> est  un  nombre
flottant supérieur à  1. Leur utilisation est expliquée  dans la suite
de ce  document. Et  l'attribut C<ref>  est une  liste d'avions  et de
...
en supposant que le fichier s'appelle F<Manfred.json>. Puis adaptez le
script  F<launch> pour  permettre le  lancement de  parties entre  des
pilotes connus.

En fait, les fichiers F<Manfred.json> et F<Roy.json> sont publiés dans
le répertoire F<Game>, parce qu'il ne contiennent rien qui soit soumis
...
en temps de choisir une manoeuvre inhabituelle S<« pour> dérouter S<l'adversaire »>
...
C<init-pilot.raku> qui lit  le fichier JSON décrivant un pilote  et qui le
convertit en objet Raku et le  stocke dans la collection C<Pilots> de
la base  de données. Et dans  mon élan, j'ai écrit  un autre programme
C<init-aircraft.raku> qui  lit le fichier  JSON décrivant  un avion et  qui le
stocke dans  la collection C<Aircraft> de  la base de données.  De plus,
...
pilote  anonyme  dans  la  collection C<Pilots>.  Les  fichiers  JSON
continuent   à  exister,   mais   ils  sont   utiles  seulement   pour
l'initialisation.

Il y  a un problème  à la  relecture des enregistrements  C<Aircraft> et
C<Pilots>. Alors que  le module C<JSON::Class> permet  de générer des
objets des classes  C<Aircraft> et C<Pilot> à partir  d'une chaîne JSON,
...
Il n'y a pas de conversion directe vers la classe C<Aircraft> ou
vers la classe C<Pilot>. Ou alors, il faut écrire un constructeur qui
extrait  chacune  des  valeurs  du hachage  C<BSON::Document>  et  qui
alimente l'attribut  correspondant de  l'objet C<Pilot>  ou C<Aircraft>.
...
j'ai  fait ici.  Lorsque le  programme crée  un document dans la collection  C<Aircraft> ou
C<Pilots> de  la base de  données, ce document contient  une entrée
C<json> qui stocke la chaîne JSON brute ayant servi à créer l'objet de
la classe C<Aircraft> ou C<Pilot>. Dans le programme suivant, lorsque je
...
qui crée  ainsi de véritables  objets C<Aircraft> ou C<Pilot>.  Dans les
...
C<Games> et C<Turns>. Ce n'est pas très cohérent, il est vrai.
...
=head3 Programme C<umpire.raku>, anciennement C<arbitre.p6>
...
=item 2 Écriture dans la collection C<Turns> des choix proposés aux deux joueurs.

=item 3 Lecture dans la collection C<Turns> de la réponse de chaque joueur.
...
=item * 2A Écriture dans la collection C<Turns> des choix proposés au joueur poursuivi.

=item * 3A Lecture dans la collection C<Turns> de la réponse du joueur poursuivi.

=item * 2B Écriture dans la  collection C<Turns> des choix proposés au
...
=item * 3B Lecture dans la collection C<Turns> de la réponse du joueur poursuivant.
...
tandis que l'avion allemand est page B<1L> ou  B<1G>, pour tenir compte du fait
...
S<antérieures »>  pour choisir  sa manoeuvre  suivante selon  qu'il est
page  B<1L>, page  B<1C> ou  page B<1R>.  Cela simplifie  le programme
...
=head4 Liste des manoeuvres disponibles
...
verrez que  j'ai prévu un  mode I<no fleeing> (pas de fuite)  où, dans le  cadre d'un
...
Cas particulier, si I<toutes> les  manoeuvres conduisent à la page 223,
...
écrit un enregistrement C<Turns> ultime  pour chacun des joueurs, avec
l'attribut C<end> (fini) à 1 et le résultat en PV (+1, -1, +1/2 ou -1/2). De
...
dans la collection C<Games>.

=head3 Programme C<player.raku>, anciennement C<joueur.p6>

Le  programme C<player.raku>  est basé  sur  une boucle  où, à  chaque
itération, il  lit un coup dans  la collection C<Turns> et  le réécrit
pour indiquer à l'arbitre la manoeuvre sélectionnée.

Chaque joueur a  deux attributs numériques, utilisés pour  le choix de
la manoeuvre suivante, la  I<psycho-rigidité> et la I<perspicacité>. La
...
Le  programme  extrait  de  la collections  C<Turns>  tous  les  coups
...
l'attribut  C<@ref> des  enregistrements de  la collection  C<Pilots>.
...
S<« Camel>  contre S<Dr.I »>  que les  coups S<« Camel>  contre Fokker
...
=head4 Évaluation des manoeuvres
...
=head4 Choix de la manoeuvre

C'est  à  ce  moment   qu'intervient  la  psycho-rigidité.  Un  pilote
psycho-rigide préférera  de loin choisir B<la>  manoeuvre la meilleure,
...
Le programme affecte des S<« poids »> à chaque manoeuvre, en prenant la
...
lorsque le programme C<player.raku>  constate que la psycho-rigidité est
...
manoeuvres disponibles et  en tire une au sort  (instruction C<pick> de
Raku).

=head4 Fin de partie

Lorsque  le  programme  C<player.raku>  lit un  document  C<Turns>  avec
l'attribut C<end> (fini)  à 1, il sait  que la partie est  terminée et qu'il
...
=head3 Programme C<website.raku> (anciennement C<site.p6>) et programmes annexes

Le programme  C<website.raku> et ses programmes  annexes C<lib/xxx-page.rakumod>
...
programme C<lib/player-turn-page.rakumod>, les informations  affichées sont un peu
compliquées,  mais elles  reproduisent le  processus décisionnel  déjà
présenté dans le programme C<player.raku>.

Les  pages générées  par les  programmes C<lib/xxx-page.rakumod>  sont
bâties  autour  d'une  liste  dynamique. Une  liste  de  parties  pour
C<lib/game-list-page.rakumod>,  une   liste  de  tours  de   jeu  pour
C<lib/game-page.rakumod>  (en fait,  il s'agit  d'un tableau,  mais le
principe est  le même) et  une liste de  manoeuvres plus une  liste de
tours    de     joueurs    pour     des    parties     passées    pour
C<lib/player-turn-page.rakumod>.   Voici  comment   ces  listes   sont
générées.

Tout d'abord, le sous-répertoire F<html> contient une version statique
...
(C<<  <li>...</li>   >>  ou  C<<  <tr>...</tr>   >>)  est  abondamment
paramétrée  avec des  attributs C<class='...'>,  pour chaque  valeur à
substituer dans la ligne. Ensuite, le module C<Template::Anti> extrait
cette  ligne,   que  j'appelle   une  I<brique>  (I<brick>   dans  les
...
structure C<< <ul></ul>  >> vide ou une  structure C<< <tbody></tbody>
>> vide, que j'appelle I<cadre>  (ou I<frame>). Puis j'applique chaque
...
la page. Ainsi  on traduit B<Attack> en B<Attaque>, et  B<L> (pour S<«
left »>) en  B<G> (pour S<« gauche »>). Les  traductions sont stockées
dans  le  fichier  HTML   statique  avec  un  attribut  C<class='...'>
permettant  de  les reconnaître  et  de  les extraire.  Elles  peuvent
apparaître dans  une ligne C<<  <tr> >>  pipeau ou dans  un paragraphe
pipeau,  lesquels   sont,  bien  évidemment,  supprimés   lors  de  la
génération de la page web dynamique.

=head2 Quelques questions
...
terme de  temps réel,  le site  I<Board Game Geek>  donne un  ordre de
...
Et une  partie S<« pour> de  S<vrai »> dure  entre 13 et 131  tours de
...
de rien. J'ai fait de même avec l'I<As des As Zéro>.
...
s'agit de jeux I<classiques> avec un  nombre énorme de joueurs. Il y a
...
étudier  les I<Dikta  Boelke> et  les règles  de Sailor  Malan. Et  ce
...
paragraphe L<Liste des manoeuvres disponibles>, il se peut que l'option
C<--no-fleeing> (pas de fuite) pour  les parties d'entraînement ait  introduit un biais
...
vous manipulez  des vecteurs  (le terme local  pour S<«  tableaux »>),
...
Et  en   fait,  L<la   fonctionnalité  de   Raku  la   plus  utile|"La
fonctionnalité de Raku la plus utile">, c'est les points communs entre
...
on utilise les  accolades les points-vigrules, les  opérateur C<my> et
C<sub>, etc.
...
les deux  jeux classiques. C'est  pour cela que  j'ai pu créer  un S<«
Alpha> As des As S<Zéro »> sur un datacenter constitué d'un seul PC de
puissance moyenne.
...
pour les combats ultérieurs. J'ai donc créé une option I<--no-fleeing> (pas de fuite) où le
choix de l'attaque est privilégié par rapport à la fuite.

Mon but avec ce projet était de montrer que les I<Dikta Boelke>  et les 
I<règles de Sailor Malan|https://fr.wikipedia.org/wiki/Sailor_Malan#Les_10_r%C3%A8gles_du_combat_a%C3%A9rien_par_Sailor_Malan>
peuvent se  déduire d'une  expérimentation du  type Alpha-I<xxx>-Zéro.
...
combats S<«  beaucoup> contre S<beaucoup »>  mais elle ne sert  à rien
dans l'I<As des  As>, où tous les  combats sont du S<« 1  contre 1 »>.
...
=head1 Annexe : à la découverte de Perl 6
...
L<Think Perl 6|https://perl.developpez.com/actu/116451/Think-Perl-6-moins-How-to-Think-Like-a-Computer-Scientist-un-livre-de-Laurent-Rosenfeld-et-Allen-B-Downey/>
...
également rebaptisé L<Learning Raku|https://www.learningraku.com/>,
par C<brian d foy>,
...
L<les regex, anciennement expressions régulières|"Les regex, anciennement expressions régulières">.

=back

Pour des requêtes ponctuelles (pas pour l'apprentissage complet),
j'utilise quand même des sites web, essentiellement la
L<documentation des types standards|https://docs.raku.org/type.html>
et la L<liste des modules|https://modules.raku.org/>.

=head2 Compilation et installation de Raku
...
Récupérer une image Docker, comme le propose Moritz Lenz dans son livre I<Perl 6 Fundamentals>.
...
=head3 La fonctionnalité de Raku la plus utile
...
Une autre version S<idiomatique :>
...
variable et créer, par exemple, une variable C<$aujourd'hui>. Voir les
exemples (en anglais) de mon
L<module de calendrier républicain|https://modules.raku.org/dist/Date::Calendar::FrenchRevolutionary:cpan:JFORGET>,
avec les variables C<$Bonaparte's-coup> et C<$Robespierre's-downfall>.
...
=head3 Deux autres héritages d'APL
...
L<module de calendrier maya|https://modules.raku.org/dist/Date::Calendar::MayaAztec:cpan:JFORGET>,
...
L<fonction  polymod|https://docs.raku.org/routine/polymod>
...
S<« un>  langage de programmation  de S<plus  »> mais bien  un domaine
dont    l'exploration    promettait   d'être    passionnante.    Cette
fonctionnalité, c'est  la possibilité  d'utiliser C<substr>  en partie
gauche d'une S<affectation :>
...
(L<Raku par example|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/search?l=raku>)
...
le répertoire  F<.zef/store>) et  je corrige  en supprimant  les trois
...
de I<bytecode> que je n'ai pas  vu. Le I<bytecode> n'ayant pas changé,
...
pas les options C<sort>, C<limit> et C<skip>. Et ce n'est pas un problème de version 2.6
...
existe encore.  En revanche, j'ai  testé le problème des  C<(Rat)>, il
est toujours là.

En 2021, j'ai  testé le problème de C<_id>, il  a disparu. Le problème
des C<(Rat)> est toujours présent.
...
F<website.raku> correspond, je pense, au Contrôleur. Dans un sous-répertoire,
j'ai F<lib/access-mongodb.rakumod>, correspondant au Modèle. Et j'ai d'autres
fichiers F<lib/player-turn-page.rakumod>, F<lib/game-list-page.rakumod> et F<lib/game-page.rakumod>
correspondant aux Vues. J'arrive à appeler les fonctions du Modèle F<lib/access-mongodb.rakumod>
depuis le Contrôleur F<website.raku>, mais pas depuis les Vues F<lib/xxx-page.rakumod>.
J'ai encore quelques notions à apprendre sur les C<use> et les C<require> en Raku.

=head2 Template::Anti

Je  n'aime pas  les  modules  de I<templating>.  Sous  le prétexte  de
...
tout plein de C<printf> et de  C<say> pour générer des balises HTML et
...
cela,  il me  fallait quand  même un  module de  I<Templating>, malgré
toutes les réticences que j'éprouve à leur égard. J'ai cherché dans le
site des modules Raku avec
L<https://modules.raku.org/search/?q=template> et j'ai découvert
L<Template::Anti|https://modules.raku.org/dist/Template::Anti:cpan:HANENKAMP>.
Le  paragraphe S<«  DESCRIPTION »>  de ce  module faisait  écho à  mes
sentiments sur les  modules de I<templates>. Je l'ai  donc essayé puis
adopté.
...
avec une paire  de balises C<< <span class>...</span> >>  pour être en
mesure de spécifier  la classe CSS. Ou vous pouvez  tirer partie d'une
balise  C<< <li>  >>,  C<<  <td> >>  ou  autre  existante. Mais  c'est
