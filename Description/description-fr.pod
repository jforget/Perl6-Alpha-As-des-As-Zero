...
pilote anonyme dans la collection C<Pilotes>.
...
C<BSON::Document> auxquels  on accède  avec la  syntaxe des  tables de
hachage. Il n'y a pas de conversion directe vers la classe C<Avion> ou
...
j'ai  fait ici.  Lorsque le  programme crée  un document  C<Avions> ou
C<Pilotes> dans  la base de  données, ce document contient  une entrée
...
C<identité> et d'une  entrée C<json>. J'ai quand  même ajouté d'autres
entrées reprenant les attributs des objets  (pas tous), pour le cas où
j'en  aurais  besoin lors  des  recherches  en  base de  données.
...
=head2 Retour d'expérience sur l'As des As
...
0 PV si l'adversaire souhaite lui aussi fuir, soit -0,25 PV en moyenne. En revanche, la poursuite
