...
=item 1 Détermination des choix accessibles aux deux joueurs et du potentiel restant (dégâts).
...
=item 5 Résolution des tirs

=item 6 Détection de la fuite d'un avion ou des deux.

=item 7 Traitement de la fin de partie (fuite ou destruction).

=back

Évidemment, si lors de l'étape 1 on détecte la destruction d'un avion,
les étapes 2 à 6 sont automatiquement sautées et on traite directement
l'étape 7.
