...
hand, the  C<JSON::Class> module  generates objects from  the C<Avion>
and  C<Pilote> classes  from  a JSON  string, on  the  other hand  the
MongoDB  modules  generate  C<BSON::Document> objects,  which  can  be
accessed  with the  same syntax  as hash  tables. There  is no  direct
conversion to  a user class  such as  C<Avion> or C<Pilote>.  Should I
write  an  object  constructor  which extracts  each  value  from  the
C<BSON::Document>  hash table  and  stores it  into the  corresponding
attribute of the C<Pilote> / C<Avion> object being created? Is there a
cleaner and simpler solution to this dilemma?
...
"identity")  and C<json>.  I have  still added  some other  entries to
store  some object  attributes,  because  they may  be  used for  some
database queries.
...
end, grant a full victory point to the enemy.
...
and for  a defeat. On average,  the win is  0 VP. So the  cyber pilots
prefer to stay.
