...
Forget the skies above Poelcapelle or Vaux-sur-Somme
...
some notions about the I<Dicta Boelke>, Sailor Malan's advices, "Boots" Blees's 
I<No Guts, No Glory> and similar books. But you can strip all this air combat decoration
and keep only the abstract mechanism, based on a finite state automaton with maybe a few
use of a I<random> function. Hence the idea of a program playing the game without any knowledge
...
Spad XIII vs Fokker D.VII) and early war planes
...
even staging a fight between a X-Wing and a Fokker E.III!
...
the  7 ± 2 short-term memory capacity. Except a rule about compatibility between
...
with a 2-vs-1 fight. The German player controls an rather passive
...
format, by generating them rather than copying them from the paper version. Second, the game
itself, where cyber pilots fight against each other.
...
into the Git repo. You will not see my very first Raku lines of code.
...
I generate a booklet, that is, the full list of associations
...
rather an umpire process, and two player processes. Both processes use the
...
where the player program search the database from similar situations encountered during the
...
stereotypical plays, each process computing a minimax best solution and always obtaining the
same choice when in the same situation. The random choice gives a higher probability to the
best answer, but this does not prevent the program from choosing an unexpected maneuver
from time to time and take the enely on his wrong foot.
