...
=item 1 Computing the possible choices and the remaining hit points for each player.

=item 2 Writing these choices into the C<Coups> (game turn) collection for each player.

=item 3 Reading back the players' choices from the C<Coups> collection.

=item 4 Applying the transitions: initial page → intermediate page → final page.

=item 5 Fire and damage computing.

=item 6 Checking whether a plane flees (or both).

=item 7 Dealing with the end of game (fleeing or destruction).

=back

Of course,  if step  1 computes that  a plane has  zero hit  points or
less, steps 2 to 6 are skipped  and we imediately deal with the end of
the game in step 7.

For game turns where an aircraft is  pursued by another, steps 2 and 3
are split in the following way:

=over 4

=item * 2A Writing these choices into the C<Coups> (game turn) collection for the pursued player.

=item * 3A Reading back the pursued player's choice from the C<Coups> collection.

=item  *  2B Writing  these  choices  into  the C<Coups>  (game  turn)
collection for  the pursuing player,  with a  G/A/D (left /  forward /
right) indication relative to the pursued player's maneuver.

=item * 3B Reading back the pursuing player's choice from the C<Coups> collection.

=back

=head2 Lessons Learned with Ace of Aces
