...
normalized to obtain a  probability distribution. This distribution is
then  converted into  a cumulative  distribution. Lastly,  the program
computes a pseudo-random number, compares it with the intervals of the
cumulative distribution and determines the chosen maneuver.
...
the idea  of a program  knowing nothing  about flying. Third,  I would
need to  scan each one  of the 223 pages  for each booklet.  A lengthy
endeavour, for a  (in my opinion) small benefit. And  for new craft or
creatures, such as a drone or a sparrowhawk, I would need to draw from
scratch the opponents.
...
Q. When an  AI learns to play  a game, such as  chess, the programmers
uses some reference books describing  the game openings or some common
techniques and tactics. Is there such a thing for I<Ace of Aces>?

A. No.  Initializing an  AI with  game openings  was how  Google Alpha
Chess was  written. But  its younger sibling  Google Alpha  Chess Zero
started from  a clean  slate. I have  done the same  with I<As  des As
Zéro>.
...
the third hand, for Ace of Aces, there are not enough players to write
books and there are  not enough players to buy and  read books. On the
other  hand, newbies  interested in  Ace of  Aces can  still read  the
biographies of  von Richthofen,  Rickenbacker and Guynemer  and others
and study the Dicta Boelke and the rules from Sailor Malan.
...
interacts with a  human instead of gathering  statistics and crunching
numbers. One problem is the time scale. I have included failsafe tests
...
microwave oven has finished warming the pizza, the baby starts to cry,
etc.  A human  opponent easily  understands what  happens, a  computer
opponent  would still  enforce  the 50-second  timeout without  second
thoughts.
